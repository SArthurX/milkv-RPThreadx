...
#define RL_PLATFORM_MAX_ISR_COUNT       (4)

/* VirtIO Ring */
#define VRING_ALIGN                     (4096)

/* Calculate VRING_SIZE based on vring_size formula */
/* vring_size = 6 + (num_descs * 16) + ALIGN(8 + (num_descs * 2), VRING_ALIGN) + ALIGN(8 + (num_descs * 8), VRING_ALIGN) */
/* For 64 descriptors at 4096 alignment: approximately 8192 bytes per vring */
#define VRING_SIZE                      (0x2000UL)  /* 4096 bytes = 0x1000 */
#define RL_VRING_OVERHEAD               (2 * VRING_SIZE) //8k


/*
 * VirtQueue ID mapping (Linux compatible): vq indices are 0 and 1.
 * Keep link_id encoded as pairs (0/1 for link0, 2/3 for link1, ...).
 */
#define RL_GET_VQ_ID(link_id, queue_id) ((uint32_t)(link_id) * 2U + (uint32_t)(queue_id))

#define DisableGlobalIRQ()   ({ \
    unsigned long __mstatus; \
    __asm__ __volatile__("csrrc %0, mstatus, %1" : "=r"(__mstatus) : "r"(0x8)); \
    __mstatus & 0x8; \
})

#define EnableGlobalIRQ(primask) do { \
    if (primask) { \
        __asm__ __volatile__("csrrs zero, mstatus, %0" :: "r"(0x8)); \
    } \
} while(0)
